...
void write_superblock(int fd) {
	...
	superblock.s_log_frag_size = LOG_BLOCK_SIZE;						/* 1024 */
	superblock.s_blocks_per_group = BLOCK_SIZE << 3;
	superblock.s_frags_per_group = BLOCK_SIZE << 3;
...
void write_hello_world_file_inode(int fd, u32 current_time)
{
...
void write_hello_symlink_inode(int fd, u32 current_time)
{
...
void write_inode_table(int fd) {
	u32 current_time = get_current_time();
	write_root_inode(fd, current_time);
	write_lost_and_found_inode(fd, current_time);
	write_hello_world_file_inode(fd, current_time);
	write_hello_symlink_inode(fd, current_time);
}
